...
class CCMatroid(CombinatorialComplex):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        independent_sets = list(
            filter(lambda X: len(X) != 0, independent_sets)
        )
    ...
    @classmethod
    def from_bases(
        cls, ground: Collection, bases: Collection | None
    ) -> "CCMatroid":
    ...
    @classmethod
    def from_circuits(
        cls, ground: Collection, circuits: Collection
    ) -> "CCMatroid":
    ...
    def matroid_rank(self, input_set: Iterable) -> int:
        ...
        raise KeyError(
            f"hyperedge {input_set} and its subsets are not in the complex"
        )
    ...
    def dual(self) -> "CCMatroid":
        ...
        """
        new_bases = [
            frozenset.difference(self.ground, base) for base in self.bases
        ]
...
class CCGraphicMatroid(CCMatroid):
    ...
    def __init__(self, edgelist: Collection, **kwargs):
        ...
        independent_sets = frozenset(
            [
                frozenset(ind)
                for base in spanning_trees
                for ind in powerset(base)
            ]
        )
        CCMatroid.__init__(
            self, ground=ground_edges, independent_sets=independent_sets
        )
